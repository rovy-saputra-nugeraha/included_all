...
$sqlOldFile = "SELECT berkas FROM tb_absen_masuk WHERE id='$id' AND date='$tanggal'";
...
if (!empty($fileName)) {
    ...
    if (in_array($fileType, $allowedFileTypes) && $fileError === 0) {
        ...
        if (move_uploaded_file($fileTmpName, $fileDestination)) {
            ...
            header("Location: ../index.php?page=absensi-masuk-guru&error=true");
            exit;
        }
    } else {
...
if ($flag === '0') {
    ...
    $sql = "UPDATE tb_absen_masuk SET status='$status', berkas='$fileNameInDatabase', keterangan='$keterangan' WHERE id='$id' AND date='$tanggal'";
} else {
    // Jika flag bukan '0', lakukan INSERT data baru
    $sql = "INSERT INTO tb_absen_masuk (id, masuk, date, status, berkas, keterangan) VALUES ('$id', '-', '$tanggal', '$status', '$fileNameInDatabase', '$keterangan')";
}
...
if (mysqli_query($dbconnect, $sql)) {
    ...
    header("Location: ../index.php?page=absensi-masuk-guru&success=true");
    exit;
} else {
