...
if (isset($_POST['tanggal'])) {
    ...
    $date = $_POST['tanggal'];
    $absent = $date;
    $sql = mysqli_query($dbconnect, "SELECT tb_absen_masuk.id, tb_id.nama, tb_id.status_kartu, tb_absen_masuk.masuk, tb_absen_masuk.date, tb_absen_masuk.status, tb_absen_masuk.keterangan FROM tb_absen_masuk INNER JOIN tb_id ON tb_absen_masuk.id=tb_id.id WHERE tb_absen_masuk.date='$date' AND tb_id.status_kartu='Siswa'");
} else {
    $flag = '0';
    $date = date('Y-m-d');
    $absent = $date;
...
<head>
    ...
    <title>Data Absensi Masuk Siswa</title>
    <style>
        thead .tabel {
            display: flex;
...
<body>
    <div class="card card-info">
        <div class="card-header">
            <h3 class="card-title">
                <i class="fa fa-table"></i> Data Absensi Masuk Siswa
            </h3>
        </div>
        <!-- /.card-header -->
        <div class="card">
                        ...
                        <div class="table table-hover">
                            ...
                            <table id="example" class="table table-striped table-bordered dt-responsive nowrap" id="dataTables-example" style="width: 100%;">
                                ...
                                <tbody>
                                    ...
                                    <?php
                                    $flag = '1';
                                    $sql1 = mysqli_query($dbconnect, "select * from tb_id where id not in(select id from tb_absen_masuk where date='$absent') AND status_kartu='Siswa'");
