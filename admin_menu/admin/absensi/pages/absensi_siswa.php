...
if (isset($_POST['tanggal'])) {
	...
	$date = $_POST['tanggal'];
	$absent = $date;
	$sql = mysqli_query($dbconnect, "SELECT tb_absen.id, tb_id.nama, tb_id.status_kartu, tb_absen.masuk, tb_absen.keluar, tb_absen.date, tb_absen.status, tb_absen.keterangan FROM tb_absen INNER JOIN tb_id ON tb_absen.id=tb_id.id WHERE date='$date' AND status_kartu='Siswa'");
} else {
	$flag = '0';
	$date = date('Y-m-d');
	$absent = $date;
