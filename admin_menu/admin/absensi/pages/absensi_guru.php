...
if (isset($_POST['tanggal'])) {
	...
	$date = $_POST['tanggal'];
	$absent = $date;
	$sql = mysqli_query($dbconnect, "SELECT tb_absen.id, tb_id.nama, tb_id.status_kartu, tb_absen.masuk, tb_absen.keluar, tb_absen.date, tb_absen.status, tb_absen.keterangan FROM tb_absen INNER JOIN tb_id ON tb_absen.id=tb_id.id WHERE date='$date' AND status_kartu='Guru'");
} else {
	$flag = '0';
	$date = date('Y-m-d');
	$absent = $date;
									...
									<tbody>
										...
										?>
											...
											<tr class="table-danger">
												...
												<td><a href="./index.php?page=edit_absen_guru&id=<?php echo $data1['id']; ?>&nama=<?php echo $data1['nama']; ?>&tanggal=<?php echo $absent; ?>&status=A&flag=<?php echo $flag; ?>">
														<center><b>A</b></center>
													</a></td>
												<td></td>
											</tr>
